-- ════════════════════════════════════════════════════════════════
-- M3: Billing — subscriptions, plan limits, usage events, webhook
-- idempotency. Master plan §6.1 (pricing), §7 (gates), §8.1, §9.
-- ════════════════════════════════════════════════════════════════

-- ── Tables ─────────────────────────────────────────────────────

-- Plan configuration (global, not tenant-owned). One row per plan.
...
-- One subscription per organization (MVP). Stripe is the source of
-- truth; the webhook keeps this row in sync.
...
-- Every billable activity (voice minutes, SMS, …) lands here. Written
-- by the server only; read by the tenant. Powers limits + overage.
...
create index usage_events_tenant_type_time_idx
  on public.usage_events (tenant_id, event_type, created_at desc);

-- Webhook idempotency (master plan §9): every Stripe event id is
-- recorded once; replays are detected by primary-key conflict.
...
-- Plan limits are public-ish pricing config: any signed-in user may read.
...
-- ── Grants (explicit; RLS still guards rows) ───────────────────
...
-- Feature flags gate modules per §6.1 "Best for" + §7. Numbers not
-- specified by the master plan (locations, retention, workflows) use
-- conservative defaults — adjust in this table anytime.
