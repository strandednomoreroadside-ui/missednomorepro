...
- ✅ M3 complete (June 2026): Stripe test-mode billing live — 5 plans (Checkout + Customer Portal), signature-verified idempotent webhook, plan_limits/usage_events seeded, feature gates; user verified subscribe → plan shows → portal works (`scripts/billing-check.mjs` to re-verify the loop)
- 🔶 M4 code complete (June 2026), pending user verification: setup wizard at `/dashboard/setup` — 10 steps, saved progress, DB-enforced launch gate (trigger + `launch_business`/`approve_setup_section` RPCs; approval stamps not writable by clients), admin sees incomplete setups. **Awaiting:** migration `20260612180000_setup_wizard.sql` pasted into SQL editor, Stran's Towing walkthrough
- 🔶 M5 code complete (June 2026), pending user verification: CRM at `/dashboard/contacts` — contacts (tags, §8.3 consent fields, phone unique per tenant for M7 caller matching), leads, notes, tamper-proof timeline (trigger-written, clients read-only). **Awaiting:** migration `20260612190000_crm.sql` pasted, leak test rerun (now 17 checks), create-contact walkthrough
