...
- 🔶 M6 code complete (June 12 2026): phone_numbers/agents/calls/call_transcripts tables (members read-only; server writes); signature-validated Twilio voice webhook `/api/twilio/voice` (greeting + voicemail-to-log, known-caller matching to CRM, timeline event on call end) + `/status` + `/recording` callbacks; voicemail playback proxy `/api/recordings/[callId]`; `/dashboard/calls` + `/dashboard/settings`; admin number assignment; `scripts/twilio-setup.mjs` points the number's webhooks at prod; `scripts/webhook-test.mjs` verifies signing. Voice decision: Retell (made earlier). **Awaiting:** migration `20260612210000_phone_foundation.sql` paste, admin assigns +14406442423, twilio-setup run, operator test call
