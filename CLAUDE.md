...
- 🔶 M3 in progress: **code complete** — Checkout, Customer Portal (API-configured), signature-verified idempotent webhook, billing migration (subscriptions/plan_limits/usage_events), feature-gate + usage-limit helpers, billing page with usage, dashboard locked-features card, and `/admin/billing-setup` (one-tap Stripe setup — operator works from phone, no local scripts). ✅ Billing migration applied in prod (June 12 2026, via the idempotent rerun copy `scripts/billing-migration-rerun.sql` — original failed with "already exists" from an earlier partial run). **Remaining (operator, phone browser):** run setup at `/admin/billing-setup` → add `STRIPE_WEBHOOK_SECRET` to Vercel + redeploy → test-subscribe with 4242 card
