...
/**
 * Low-level Twilio SMS send. Returns the MessageSid so the caller can
 * correlate delivery-status callbacks.
 *
 * Sender selection (in order):
 *  1. `from` — the tenant's OWN number. Passed by the outbound layer so every
 *     business texts from its own line (brand identity + multi-tenant safety:
 *     never send business A's text from business B's number). The number is
 *     registered in the approved A2P Messaging Service, so it keeps 10DLC
 *     deliverability.
 *  2. the A2P Messaging Service — fallback for a tenant with no number yet.
 *  3. TWILIO_PHONE_NUMBER — last-resort platform number.
 *
