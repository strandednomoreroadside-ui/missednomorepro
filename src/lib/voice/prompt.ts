...
Today is {{current_day}}, {{current_date}} in the business's local time. Use it to work out any date the caller mentions (e.g. "next Tuesday").\n`
  ...
  const steps: string[] = [
    ...
    !mobileService
      ? `Capture the need, one question at a time and as efficiently as possible: what service they're looking for (and any day, time, or team member they prefer)${vehicleIntake ? ", the vehicle's year, make, and model" : ""}, and the best callback number. Customers come to the business, so do NOT ask for a street address or where they are.`
      : vehicleIntake
  ...
  if (bookingEnabled) {
    steps.push(
      (mobileService
        ...
        : 'Booking: customers come to the business, so nothing is ever dispatched to them: never say help or someone is "on the way", and never call notify_staff with urgency "high" or "emergency" just because they want to come in soon. If they want to come in right now or today, check today first and offer the soonest open times; never promise a walk-in spot. To book: ') +
