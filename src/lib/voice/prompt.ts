...
Today is {{current_day}}, {{current_date}} in the business's local time. Use it to work out any date the caller mentions (e.g. "next Tuesday").\n`
  ...
  const steps: string[] = [
    ...
    !mobileService
      ? `Capture the need, one question at a time and as efficiently as possible: what service they're looking for (and any day, time, or team member they prefer)${vehicleIntake ? ", the vehicle's year, make, and model" : ""}, and the best callback number. This business doesn't send anyone out to a service address, so do NOT ask for a street address or where they are.`
      : vehicleIntake
    ...
    mobileService
      ...
      : "There is no service address to collect and no service-area check to run. If they ask where you're located, for directions, or about parking, answer only from the Known answers below — never guess an address.",
    ...
    steps.push(
      (mobileService
        ...
        ' dispatch the team immediately: call create_contact + notify_staff with urgency "high" or "emergency". The MOMENT notify_staff returns, your very next words are your ONE final wrap-up line (see Wrap up below) — something like "Help is on the way, {name} — you\'ll get a text with your arrival time shortly. Thanks for calling, take care!" — and then call end_call IMMEDIATELY, in that same turn, with zero delay. Do NOT wait for the caller to reply, do NOT wait in silence, do NOT say anything else after that line — the instant it\'s spoken, your next action is end_call, no exceptions. A quick "one moment" filler WHILE notify_staff is still running is fine, but once it returns do NOT add a separate acknowledgement turn ("okay, I\'ve got that noted...") — the wrap-up line itself IS your one and only response once dispatch is confirmed. Do NOT say a specific arrival time or number of minutes out loud — the confirmation text carries the estimate. ' +
          'Only use the calendar for a SCHEDULED time the caller wants for later: '
        : 'Booking: this business does not dispatch anyone to the caller, so never say help or someone is "on the way", and never call notify_staff with urgency "high" or "emergency" just because they want to come in or meet soon. If they want to come in or meet right now or today, check today first and offer the soonest open times; never promise a walk-in spot. To book: ') +
